...
from app.schemas import Claim, QaResult, Report, Task
...
class ReportWriterAgent:
    ...
    def run(
        ...
        def produce() -> dict:
            ...
            raw_claims = [
                {
                    ...
                    "evidence_ids": [] if simulate_missing_evidence else ids,
                    "confidence": 0.84,
                },
                {
            ...
            if simulate_missing_evidence:
                return {"draft_report": {"claims": raw_claims, "markdown": "# 草稿\n\n存在缺少证据绑定的无效结论。"}}
            ...
            report = Report(
                ...
                markdown=markdown,
                json_report={"knowledge": knowledge, "claims": [claim.model_dump(mode="json") for claim in claims]},
                claims=claims,
